...
const NavigationLink = React.forwardRef<HTMLAnchorElement, NavigationLinkProps>(
  (
    { className, pathname, href, activeOnSubpath = false, children, ...props },
    ref
  ) => {
...
const Navbar = React.forwardRef<HTMLDivElement, HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => {
    ...
    return (
      <nav
        className={cn(
          'sticky top-0 w-full min-h-24 md:min-h-20 flex flex-col md:flex-row items-center justify-center gap-2 md:px-14 lg:px-20 xl:px-24 2xl:px-28 shadow-md bg-secondary-500 transform duration-300 ease-in-out',
        ...
        <div className='w-full h-full flex flex-row justify-center md:justify-end items-center'>
          <ul className='w-fit h-full flex flex-row justify-center items-center gap-3'>
            <li className='w-fit h-fit'>
              <NavigationLink href='/home' pathname={pathname} activeOnSubpath>
                Home
              </NavigationLink>
            </li>
            <li className='w-fit h-fit'>
